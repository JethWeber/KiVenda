...
file sealed class FakeProdutoRepository(InMemoryDatabase db) : IProdutoRepository
{
    ...
    public Task<IReadOnlyList<Produto>> ListarAsync(string? termoPesquisa = null, Guid? categoriaId = null, bool apenasAtivos = true, CancellationToken ct = default)
    {
        ...
        if (!string.IsNullOrWhiteSpace(termoPesquisa)) query = query.Where(p => p.Nome.Contains(termoPesquisa, StringComparison.OrdinalIgnoreCase));
        return Task.FromResult<IReadOnlyList<Produto>>(query.ToList());
    }
    public Task AdicionarAsync(Produto produto, CancellationToken ct = default) { db.Produtos.Add(produto); return Task.CompletedTask; }
}

file sealed class FakeCategoriaRepository(InMemoryDatabase db) : ICategoriaRepository
{
    public Task<Categoria?> ObterPorIdAsync(Guid id, CancellationToken ct = default) => Task.FromResult(db.Categorias.FirstOrDefault(c => c.Id == id));
    public Task<IReadOnlyList<Categoria>> ListarAsync(CancellationToken ct = default) => Task.FromResult<IReadOnlyList<Categoria>>(db.Categorias.ToList());
    public Task AdicionarAsync(Categoria categoria, CancellationToken ct = default) { db.Categorias.Add(categoria); return Task.CompletedTask; }
}

file sealed class FakeUnidadeMedidaRepository(InMemoryDatabase db) : IUnidadeMedidaRepository
{
    public Task<UnidadeMedida?> ObterPorIdAsync(Guid id, CancellationToken ct = default) => Task.FromResult(db.UnidadesMedida.FirstOrDefault(u => u.Id == id));
    public Task<UnidadeMedida?> ObterPorCodigoAsync(string codigo, CancellationToken ct = default) => Task.FromResult(db.UnidadesMedida.FirstOrDefault(u => u.Codigo == codigo));
    public Task<IReadOnlyList<UnidadeMedida>> ListarAsync(CancellationToken ct = default) => Task.FromResult<IReadOnlyList<UnidadeMedida>>(db.UnidadesMedida.ToList());
    public Task AdicionarAsync(UnidadeMedida unidadeMedida, CancellationToken ct = default) { db.UnidadesMedida.Add(unidadeMedida); return Task.CompletedTask; }
}

file sealed class FakeMovimentoStockRepository(InMemoryDatabase db) : IMovimentoStockRepository
{
    public Task AdicionarAsync(MovimentoStock movimento, CancellationToken ct = default) { db.MovimentosStock.Add(movimento); return Task.CompletedTask; }
    public Task<IReadOnlyList<MovimentoStock>> ListarPorProdutoAsync(Guid produtoId, DateTime? de = null, DateTime? ate = null, int pagina = 1, int tamanhoPagina = 50, CancellationToken ct = default)
    {
        ...
        return Task.FromResult<IReadOnlyList<MovimentoStock>>(query.OrderByDescending(m => m.Data).Skip(pagina1Indexada * tamanhoPagina).Take(tamanhoPagina).ToList());
    }
    public Task<IReadOnlyList<MovimentoStock>> ListarTodosPorProdutoAsync(Guid produtoId, CancellationToken ct = default) => Task.FromResult<IReadOnlyList<MovimentoStock>>(db.MovimentosStock.Where(m => m.ProdutoId == produtoId).OrderBy(m => m.Data).ToList());
}
...
{
    public Task<Compra?> ObterPorIdAsync(Guid id, CancellationToken ct = default) => Task.FromResult(db.Compras.FirstOrDefault(c => c.Id == id));
    public Task<IReadOnlyList<Compra>> ListarAsync(Guid? fornecedorId = null, DateTime? de = null, DateTime? ate = null, CancellationToken ct = default) => Task.FromResult<IReadOnlyList<Compra>>(db.Compras.Where(c => !fornecedorId.HasValue || c.FornecedorId == fornecedorId.Value).ToList());
    public Task AdicionarAsync(Compra compra, CancellationToken ct = default) { db.Compras.Add(compra); return Task.CompletedTask; }
}

file sealed class FakeVendaRepository(InMemoryDatabase db) : IVendaRepository
{
    public Task<Venda?> ObterPorIdAsync(Guid id, CancellationToken ct = default) => Task.FromResult(db.Vendas.FirstOrDefault(v => v.Id == id));
    public Task<IReadOnlyList<Venda>> ListarAsync(Guid? utilizadorId = null, Guid? clienteId = null, DateTime? de = null, DateTime? ate = null, CancellationToken ct = default)
    {
        ...
        if (utilizadorId.HasValue) query = query.Where(v => v.UtilizadorId == utilizadorId.Value);
        if (clienteId.HasValue) query = query.Where(v => v.ClienteId == clienteId.Value);
        if (de.HasValue) query = query.Where(v => v.Data >= de.Value);
        if (ate.HasValue) query = query.Where(v => v.Data <= ate.Value);
        return Task.FromResult<IReadOnlyList<Venda>>(query.ToList());
    }
    public Task AdicionarAsync(Venda venda, CancellationToken ct = default) { db.Vendas.Add(venda); return Task.CompletedTask; }
}

file sealed class FakeSessaoCaixaRepository(InMemoryDatabase db) : ISessaoCaixaRepository
{
    public Task<SessaoCaixaEntity?> ObterPorIdAsync(Guid id, CancellationToken ct = default) => Task.FromResult(db.SessoesCaixa.FirstOrDefault(s => s.Id == id));
    public Task<SessaoCaixaEntity?> ObterAbertaAsync(CancellationToken ct = default) => Task.FromResult(db.SessoesCaixa.FirstOrDefault(s => s.Estado == KiVenda.Core.Enums.EstadoSessaoCaixa.Aberta));
    public Task<IReadOnlyList<SessaoCaixaEntity>> ListarAsync(DateTime? de = null, DateTime? ate = null, CancellationToken ct = default) => Task.FromResult<IReadOnlyList<SessaoCaixaEntity>>(db.SessoesCaixa.ToList());
    public Task AdicionarAsync(SessaoCaixaEntity sessaoCaixa, CancellationToken ct = default) { db.SessoesCaixa.Add(sessaoCaixa); return Task.CompletedTask; }
}

file sealed class FakeUtilizadorRepository(InMemoryDatabase db) : IUtilizadorRepository
{
    public Task<Utilizador?> ObterPorIdAsync(Guid id, CancellationToken ct = default) => Task.FromResult(db.Utilizadores.FirstOrDefault(u => u.Id == id));
...
{
    public Task AdicionarAsync(LogAuditoria log, CancellationToken ct = default) { db.LogsAuditoria.Add(log); return Task.CompletedTask; }
    ...
    {
        var query = db.LogsAuditoria.AsEnumerable();
        if (utilizadorId.HasValue) query = query.Where(l => l.UtilizadorId == utilizadorId.Value);
        if (!string.IsNullOrWhiteSpace(entidadeAfetada)) query = query.Where(l => l.EntidadeAfetada == entidadeAfetada);
        if (!string.IsNullOrWhiteSpace(acao)) query = query.Where(l => l.Acao == acao);
        if (de.HasValue) query = query.Where(l => l.DataHora >= de.Value);
        if (ate.HasValue) query = query.Where(l => l.DataHora <= ate.Value);
        var pagina1Indexada = Math.Max(0, pagina - 1);
        return Task.FromResult<IReadOnlyList<LogAuditoria>>(query.OrderByDescending(l => l.DataHora).Skip(pagina1Indexada * tamanhoPagina).Take(tamanhoPagina).ToList());
    }
}
